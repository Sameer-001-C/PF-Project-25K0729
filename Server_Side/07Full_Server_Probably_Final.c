            ...
            switch(Server_Code){
                ...
                case '4': //send message to someone format: ---> <Server_Code><Sender_UserID><Reciever_UserID>
                    ...
                    if(Online_Reciever_Index == -1){ //reciever is offline
                        ...
                        char reply[1] =  '9'; //tell the sender that his message would be delievered successfully
                        send(client_socket, reply, strlen(reply), 0);
                        closesocket(client_socket); //close socket with the sender
