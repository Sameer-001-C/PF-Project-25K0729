...
int main() {
    ...
    strcat(client_dir_path, client_username);  // Full path to client folder
    ...
    strcpy(chats_dir_path, client_dir_path);
    strcat(chats_dir_path, "/Chats");

    // Check if Chats folder exists
    if (_access(chats_dir_path, 0) == 0) {
        printf("Directory already exists.\n");
    } else {
        int status = _mkdir(chats_dir_path);
        if (status == 0) {
            printf("Directory created successfully.\n");
        } else {
            perror("Error creating directory\n");
    ...
    strcpy(contacts_file_path, chats_dir_path);
    strcat(contacts_file_path, "/Address.txt");
    ...
    strcpy(friend_chat_file_path, chats_dir_path);
    strcat(friend_chat_file_path, "/");
    strcat(friend_chat_file_path, friend_user_id);
    strcat(friend_chat_file_path, ".txt");
