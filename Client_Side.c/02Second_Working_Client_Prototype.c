//This does not include the directory handling yet
//Fully working code tho, works with "Full_Server_Probably_Final_Updated"
//Laptop pai chal rhi hai yeh script, pc pai server wali script chal rhi hai
...
#define SERVER_IP "192.168.0.113" // Change this to your server's IP address (Sameer k computer ka ip address hai yeh)
#define PORT 8080
...
void handle_login() {
    ...
    switch (code) {
        case 1:
            printf("\n[SUCCESS] Client Code 1: Login successful. Welcome, %s!\n", user_id);
...
void handle_create_account() {
    ...
    switch (code) {
        case 1:
            printf("\n[SUCCESS] Client Code 1: Account created and logged in successfully. Welcome, %s!\n", user_id);
            strcpy(current_user_id, user_id);
            break;
        case 3:
...
void handle_send_message() {
    ...
    switch (code) {
        case 9:
            printf("\n[SUCCESS] Client Code 9: Message delivered to server for forwarding (Receiver is online).\n");
            break;
        case 6:
...
void handle_logout() {
    if (current_user_id[0] == 0) {
        // This should not happen if called from the Logged In menu, but safe guard it.
    ...
    // We expect send_request_and_get_response to return 0 (due to no reply).
    int code = send_request_and_get_response(request, strlen(request), response, sizeof(response));
...
/**
 * @brief Prints the menu options based on the client's current login state.
 */
void print_menu() {
    printf("\n===================================\n");
    if (current_user_id[0] != 0) {
        ...
        printf("===================================\n");
        printf("1. Add Friend (Server Code 3)\n");
        printf("2. Send Message (Server Code 4)\n");
        printf("3. Logout (Server Code 5)\n");
        printf("4. Exit Client\n");
    } else {
        // Logged Out Menu
        ...
        if (current_user_id[0] != 0) {
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    handle_logout(); // Clears current_user_id
                    break;
                case 4:
            ...
            switch (choice) {
                case 1:
                    handle_login(); // Sets current_user_id on success
                    break;
                case 2:
                    handle_create_account(); // Sets current_user_id on success
                    break;
                case 3:
                    running = 0;
                    break;
